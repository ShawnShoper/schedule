...
import static org.shoper.taskScript.script.MFW_Hotelcommons.timeUnit;
 ...
 * @author ShawnShoper
 */
public class Sina_weibo_bak extends JobCaller {
...
//		long count = getMongoModule().getMongoTemplate()
//				.count(Query.query(
//						Criteria.where("fid").is(getArgs().getJobCode())),
//						"datas");
//		if (count != 0)
...
//	}
     ...
     * @throws InterruptedException
     */
    protected void fetchPage() throws InterruptedException, TimeoutException, HttpClientException {
        // 读取首页获取相关参数...
        ...
        for (; ; ) {
            ...
            try {
                httpClient = HttpClientBuilder.custom().setUrl(q).setProxy(true).setTimeout(timeout).setTimeoutUnit(TimeUnit.SECONDS).setRetry(retry).setCharset(charset).build();
            ...
            String json = httpClient.doGet();
            if (json == null) {
                logger.info("读取数据异常,请检查网络.....");
                break;
            }
            Document doc = null;
            System.out.println("当前页码数:" + page);
            JSONObject jsonObject = JSONObject.parseObject(json);
            String data = jsonObject.getString("data");
            if (StringUtil.isEmpty(data)) continue;
            doc = Jsoup.parse(data);
//                System.out.println(doc);
            while (!analyzeQueue.offer(doc, timeout, timeoutUnit)) ;
            // check has more page
            if (!hasMorePage(doc, page)) {
                logger.info("Docment has no more page,so quit...");
                break;
            }
            // 模拟人类看微博的习惯..一页需要看几秒甚至几分钟
            TimeUnit.SECONDS.sleep(5);//(int) (Math.random() * 1) + 10);
            // 根据 weibo 的参数规则
     ...
     * @return
     */
    boolean hasMorePage(Document document, int cp) {
        ...
        if (page_emts == null || page_emts.isEmpty()) {
            System.out.println("page_emts is empty...");
            return false;
        }
        Elements pageList = page_emts.get(0).getElementsByAttributeValue("action-type", "feed_list_page_morelist");
        if (pageList == null || pageList.isEmpty()) {
            System.out.println("pageList is empty...");
            System.out.println(document.toString());
            return false;
        }
        Elements pages = pageList.get(0).getElementsByAttributeValue("action-type", "login");
        if (pages == null || pages.isEmpty()) {
            System.out.println("pages is empty...");
            System.out.println(document.toString());
            return false;
        }
        int total = Integer.parseInt(pages.get(0).text().replaceAll("第|页| ", ""));
        int cur = Integer.parseInt(pages.get(pages.size() - 1).text().replaceAll("第|页| ", ""));
    ...
    private Config injectConfig() {
        ...
        try {
            HttpClient httpClient = null;
            httpClient = HttpClientBuilder.custom().setUrl(ROOTURL + (args.getType() == null
                    ? ""
                    : args.getType() + "/") + args.getJobCode()).setProxy(true).setTimeout(timeout).setTimeoutUnit(timeUnit).setRetry(retry).setCharset(charset).build();
            Map<String,String> requestHeader = new HashMap<String, String>();
            requestHeader.put("Proxy-Switch-Ip","yes");
            httpClient.setRequestHeader(requestHeader);
            httpClient.setCookies(args.getCookies());
            Document main = httpClient.getDocument();
